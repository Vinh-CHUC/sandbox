            ...
            if self.result == Equation.eval(self.operands, c):
                if debug:
                    ...
                    print(
                        reduce(
                            lambda acc, el: acc
                            + f"{str(el[0])}{' ' + str(el[1]) + ' ' if el[1] else ''}",
                            x,
                            "",
                        )
                    )
            ...
            match operators:
                case [Operator.ADD, *rest_operators]:
                    return Equation.eval(
                        [operands[0] + operands[1], *operands[2:]], rest_operators
                    )
...
def get_data() -> list[Equation]:
    with (Path(__file__).parent / "_7_bridge_repair.dat").open() as f:
        return [Equation.parse(l.strip()) for l in f.readlines()]
