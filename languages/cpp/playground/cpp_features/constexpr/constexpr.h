...
template <typename T, T (*F)(), typename = void> struct ConstExprTester {
...
constexpr int one() { return ONE; }

int two() { return TWO; }

constexpr std::vector<int> vec_builder() { return std::vector<int>{}; }
...
template <typename T, T t> struct ConstExprTester2 {
  static const bool is_valid = false;
};

// This simply doesn't build at all as two() is not a constexpr and this is
// passed directly
...
// This would always work provided that F() is a function of the right type
// (regardless of constexpr-ness) Because decltype only deduces the type of F(),
// it does not attempt to evaluate it, which is why constexpr-ness is irrelevant
/* template <typename T, T(*F)()> */
